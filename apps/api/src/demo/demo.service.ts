...
import { Requirement, RequirementState } from '../requirements/requirement.entity';
...
export class DemoService {
    ...
    async createDemoProject(tenantId: string) {
        ...
        const reqs = [
            {
                ...
                rqs: { score: 90, clarity: 95, completeness: 85, testability: 90, consistency: 90, feedback: [] }
            },
            {
            ...
            },
            {
                ...
                rqs: { score: 45, clarity: 50, completeness: 40, testability: 45, consistency: 50, feedback: ['Ambiguous payment gateway details'] }
            },
            {
                title: 'User Profile',
                content: 'Users can update their profile picture.',
                state: RequirementState.READY,
                rqs: { score: 85, clarity: 90, completeness: 80, testability: 85, consistency: 85, feedback: [] }
            }
        ];
        ...
        // 4. Create Bugs
        const bugs = [
            { title: 'Login fails on Safari', description: 'Clicking login button does nothing.', severity: BugSeverity.HIGH, priority: BugPriority.P1, status: BugStatus.NEW, requirementId: savedReqs[0].id },
            { title: 'Typo in Checkout', description: 'Label says "Paymeny"', severity: BugSeverity.LOW, priority: BugPriority.P3, status: BugStatus.RESOLVED, requirementId: savedReqs[2].id },
        ];

        for (const b of bugs) {
            await this.bugRepo.save(this.bugRepo.create({ ...b, tenantId: tenantId }));
        }
        ...
        // 7. Synthetic AI Logs
        const logs = [
            { action: 'ANALYZE_REQ', provider: 'FOUNDRY', latencyMs: 1200, success: true, model: 'gpt-4o' },
            { action: 'GENERATE_TESTS', provider: 'FOUNDRY', latencyMs: 2500, success: true, model: 'gpt-4o' },
        ];
        for (const l of logs) {
