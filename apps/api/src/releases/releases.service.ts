...
export class ReleasesService {
    ...
    async create(version: string, tenantId: string, env: string = 'staging'): Promise<Release> {
    ...
    async findAll(tenantId: string): Promise<Release[]> {
        return this.releasesRepository.find({ where: { tenantId }, order: { createdAt: 'DESC' } });
    }
    ...
    async update(id: string, data: Partial<Release>, tenantId: string): Promise<Release> {
