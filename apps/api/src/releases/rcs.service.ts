...
export class RcsService {
    ...
    async evaluateReleaseGates(releaseId: string, tenantId: string, overrideReason?: string) {
        // Get RCS Score
        ...
        const criticalBugsGate = {
            ...
            threshold: 0,
            message: criticalBugs.length === 0
                ? 'No open critical bugs'
                : `${criticalBugs.length} open critical bug(s) must be resolved`,
            details: criticalBugs.map(b => ({ id: b.id, title: b.title, priority: b.priority })),
        };
        ...
        const testCoverageGate = {
            ...
            threshold: 80,
            message: rcsResult.breakdown.qt >= 80
        ...
        const highBugs = bugs.filter(
            b => b.severity === 'HIGH' && b.status !== 'RESOLVED' && b.status !== 'CLOSED'
        );
        const highBugsGate = {
