...
export class AutomationCandidateService {
    ...
    async getCandidates(tenantId: string, projectId: string): Promise<CandidateWithMetrics[]> {
        ...
        for (const candidate of candidates) {
            const testCase = await this.testCaseRepo.findOne({ where: { id: candidate.testCaseId } });
            if (!testCase) continue;

            const metrics = await this.calculateTestMetrics(candidate.testCaseId, tenantId);
    ...
    async getAutomationCandidatesWithAI(tenantId: string, projectId: string, limit: number = 10): Promise<CandidateWithMetrics[]> {
        ...
        for (const testCase of allTestCases) {
            ...
            if (metrics.automationScore >= 50) {
                candidatesWithScores.push({
                    ...
                    testCaseTitle: testCase.title,
                    status: testCase.isAutomationCandidate ? CandidateStatus.NOT_STARTED : CandidateStatus.NOT_STARTED,
                    automationScore: metrics.automationScore,
                    executionCount: metrics.executionCount,
                    passRate: metrics.passRate,
                    aiRecommendation: metrics.aiRecommendation,
    ...
    private async calculateTestMetrics(testCaseId: string, tenantId: string) {
        ...
        const passRate = executionCount > 0 ? Math.round((passCount / executionCount) * 100) : 0;
        ...
        if (testCase?.steps && testCase.steps.length > 0) {
            ...
            if (stepCount >= 5 && stepCount <= 15) automationScore += 30; // Sweet spot
        ...
        if (automationScore >= 80) aiRecommendation = 'Excellent candidate - High ROI';
        else if (automationScore >= 60) aiRecommendation = 'Good candidate - Moderate ROI';
        else if (automationScore >= 40) aiRecommendation = 'Fair candidate - Consider effort';
    ...
    async getAutomationCoverage(tenantId: string, projectId: string) {
        ...
        const candidates = await this.candidateRepo.find({ where: { tenantId, projectId } });
        const prOpen = candidates.filter(c => c.status === CandidateStatus.PR_OPEN).length;
        const merged = candidates.filter(c => c.status === CandidateStatus.MERGED).length;

        return {
            totalTests,
            automatedTests,
            automationRate: totalTests > 0 ? Math.round((automatedTests / totalTests) * 100) : 0,
            candidates: candidates.length,
            prOpen,
            merged,
            completionRate: candidates.length > 0 ? Math.round((merged / candidates.length) * 100) : 0,
        };
    }
}
