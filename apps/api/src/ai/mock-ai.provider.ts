...
`;
    ...
    async explainRcs(releaseInfo: { score: number; breakdown: any }): Promise<{
        ...
        if (breakdown.rp < 50) {
            risks.push('Many requirements are not in READY state - consider completing requirement reviews');
        } else if (breakdown.rp >= 80) {
            strengths.push('Requirements planning is solid with most items in READY state');
        }

        if (breakdown.qt < 70) {
            risks.push('Test pass rate is below target - investigate failing tests before release');
        } else if (breakdown.qt >= 90) {
            strengths.push('Excellent test coverage with high pass rate');
        }

        if (breakdown.b < 70) {
            risks.push('Open bugs are impacting release confidence - prioritize critical defect resolution');
