...
export class FoundryAiProvider implements AiProvider {
    ...
    private readonly apiUrl = process.env.LLM_API_ENDPOINT || 'https://api.openai.com/v1/chat/completions';
    ...
    async generateTestCode(testCase: { title: string; steps: any[] }, framework: string): Promise<string> {
        const stepsStr = testCase.steps.map(s => \`- \${s.step} (Expect: \${s.expected})\`).join('\\n');
    ...
    async explainRcs(releaseInfo: { score: number; breakdown: any }): Promise<any> {
        const prompt = PROMPTS.EXPLAIN_RCS(releaseInfo.score, JSON.stringify(releaseInfo.breakdown, null, 2));
    ...
    private async callLlmRaw(prompt: string, config: any, action: string): Promise<string> {
        ...
        try {
            ...
            throw error;
             ...
             this.aiMetrics.logUsage('unknown', action, 'FOUNDRY', duration, success).catch(e => console.error('Metrics log failed', e));
        }
    }
}
