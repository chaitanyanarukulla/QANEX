...
export class LocalAiProvider implements AiProvider {
    ...
    async analyzeRequirement(content: string): Promise<{
        ...
        try {
            const result = await this.localModelGateway.complete({
                model: 'llama3:instruct', // Example default, should probably be configurable
                prompt: prompt,
            ...
            this.logger.error('Failed to analyze requirement locally', error.stack);
            // Fallback
            return {
                score: 0,
                clarity: 0,
    ...
    async triageBug(bugValues: { title: string; description: string }): Promise<{
        ...
        try {
            ...
            this.logger.error('Failed to triage bug locally', error.stack);
            return {
    ...
    async generateTestCode(testCase: { title: string; steps: any[] }, framework: string): Promise<string> {
        ...
        try {
            return await this.localModelGateway.complete({
                model: 'llama3:instruct', // Use a code-specialized model if available
                prompt: prompt,
                maxTokens: aiConfig.tasks.codeGeneration.maxTokens,
                temperature: aiConfig.tasks.codeGeneration.temperature,
            });
        } catch (error) {
            this.logger.error('Failed to generate test code locally', error.stack);
