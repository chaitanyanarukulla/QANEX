...
export class SecurityOpsService {
  ...
  async calculateSoScore(
      ...
      if (
        ...
        if (
          ...
          (check.completedAt &&
            ...
            check.completedAt > existing.completedAt)
        ) {
    ...
    const totalScore = completedChecks.reduce(
      (sum, c) => sum + (c.score || 0),
      0,
    );
    const avgScore = totalScore / completedChecks.length;

    const passedCount = completedChecks.filter(
      (c) => c.status === CheckStatus.PASSED,
    ).length;
