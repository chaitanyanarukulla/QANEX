import { Injectable, OnModuleInit, NotFoundException, Logger, Inject } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { Plan, PlanKey } from './plan.entity';
import { Subscription, SubscriptionStatus, BillingProviderType } from './subscription.entity';
...
export class SubscriptionService implements OnModuleInit {
        ...
        const plans = [
            {
                ...
                maxUsers: 5,
                maxProjects: 2,
                featureFlags: { rcsGates: false, automationPrs: false, localAiMode: false },
            },
            {
                key: PlanKey.PRO,
                name: 'Pro',
                ...
                maxProjects: null,
                featureFlags: { rcsGates: true, automationPrs: true, localAiMode: false },
            },
            {
                ...
                maxUsers: null,
                maxProjects: null,
                featureFlags: { rcsGates: true, automationPrs: true, localAiMode: true },
            },
        ];
    ...
    async createTrialSubscription(tenantId: string, email: string, name: string): Promise<Subscription> {
        // 1. Get Starter Plan
        const plan = await this.planRepository.findOne({ where: { key: PlanKey.STARTER } });
        if (!plan) throw new Error('Starter plan not found');

        // 2. Create Billing Customer (optional for Starter/Free but good practice)
        let customerId;
        try {
            const result = await this.billingProvider.createCustomer({ tenantId, email, name });
    ...
    async getSubscription(tenantId: string): Promise<Subscription> {
        ...
        if (!sub) {
            ...
            const plan = await this.planRepository.findOne({ where: { key: PlanKey.STARTER } });
            if (!plan) throw new Error('Starter plan not found during fallback subscription creation');
    ...
    async getPlanLimits(tenantId: string): Promise<{ maxUsers: number | null; maxProjects: number | null; flags: any }> {
