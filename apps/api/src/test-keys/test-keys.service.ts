...
export class TestKeysService {
  ...
  async getTestRunWithStats(
    id: string,
    tenantId: string,
  ): Promise<TestRunWithStats> {
  ...
  async getTestRunStats(
    ...
    const failed = results.filter(
      (r) => r.status === TestResultStatus.FAIL,
    ).length;
    const blocked = results.filter(
      (r) => r.status === TestResultStatus.BLOCKED,
    ).length;
    const skipped = results.filter(
      (r) => r.status === TestResultStatus.SKIPPED,
    ).length;
  ...
  async recordResult(
    ...
    const existing = await this.testResultRepo.findOne({
      where: { runId, caseId, tenantId },
    });
