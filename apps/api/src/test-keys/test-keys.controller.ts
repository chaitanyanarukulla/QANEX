...
export class TestKeysController {
    ...
    // --- Test Runs ---
    @Post('runs')
    createRun(@Body() dto: { name: string }, @Request() req: AuthenticatedRequest) {
    ...
    recordResult(
        ...
        return this.testKeysService.recordResult(runId, dto.caseId, dto.status, req.user.tenantId, dto.notes);
    }
